...
class Points:
  ...
  def check_input(self):
    ...
    if self.labels_2 is not None:
      assert self.labels_2.dim() == 2 or self.labels_2.dim() == 1
      assert self.labels_2.size(0) == self.points.size(0)
      if self.labels_2.dim() == 1:
        self.labels_2 = self.labels_2.unsqueeze(1)
  ...
  def __getitem__(self, mask: torch.Tensor):
    ...
    if self.labels_2 is not None:
      out.labels_2 = self.labels_2[mask]
    if self.batch_id is not None:
  ...
  def to(self, device: Union[torch.device, str], non_blocking: bool = False):
    ...
    if self.labels_2 is not None:
      points.labels_2 = self.labels_2.to(device, non_blocking=non_blocking)
    if self.batch_id is not None:
  ...
  def save(self, filename: str, info: str = 'PNFL'):
    ...
    mapping = {
        ...
        'F': ('features', self.features), 'L': ('labels', self.labels),'L_2': ('labels_2', self.labels_2),
...
def merge_points(points: List['Points'], update_batch_info: bool = True):
  ...
  if points[0].labels_2 is not None:
    out.labels_2 = torch.cat([p.labels_2 for p in points], dim=0)
